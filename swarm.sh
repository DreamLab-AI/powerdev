...
setup_run_opts() {
  RUN_OPTS=(
    ...
    -u dev
    --env-file "${ENVFILE}"
    -v "${EXTERNAL_DIR:-./.swarm-docker/ext}:/workspace/ext"
    -v "./.swarm-docker/docker_data:/home/dev/data"
    -v "./.swarm-docker/docker_workspace:/home/dev/workspace"
    -v "./.swarm-docker/docker_analysis:/home/dev/analysis"
    -v "./.swarm-docker/docker_logs:/home/dev/logs"
    -v "./.swarm-docker/docker_output:/home/dev/output"
...
preflight() {
  ...
  mkdir -p "./.swarm-docker/docker_analysis" "./.swarm-docker/docker_logs" "./.swarm-docker/docker_output" "./.swarm-docker/ext"
...
RUN python3.12 -m venv /opt/venv312 && \
    ...
    # Note: Placeholder npm packages commented out as they don't exist in registry
    # npm install -g claude-flow@2.0.0
RUN npm install -g ruv-swarm @anthropic-ai/claude-code @openai/codex @google/gemini-cli
...
# Simplified CMD since the npm packages don't exist yet
CMD ["/bin/bash"]
DOCKERFILE_EOF
